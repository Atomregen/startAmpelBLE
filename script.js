...
let isWriting = false; // Sperre für gleichzeitiges Senden
...
async function sendDataCmd(cmd) {
    ...
    // Warte, falls gerade gesendet wird
    while(isWriting) {
...
async function fetchAndUpload(identifier, mode) {
    ...
    // Filtern (Zukunft oder max 5 Min vergangen)
...
async function updateScheduleUI(races) {
    ...
    if(races.length === 0) {
        ...
        // --- HIER WURDE DIE DAUER HINZUGEFÜGT ---
        listEl.innerHTML = races.map(s => {
            // Berechne Dauer für Anzeige (z.B. "5 min" oder "05:30")
            ...
            <div class="schedule-item">
                <span>${s.name} <span style="color:#aaa; font-size:0.9em;">(${durStr})</span></span>
